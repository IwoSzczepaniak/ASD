...
def bfs(s, sink, parents, residual, ver):
    visited = [False for _ in range(sink)]
    visited[s] = True
    parents[s] = -1
    d_queue = deque()
    d_queue.append(s)
...
def ford_fulk(graph, source, sink, ver):
    parent = [-1 for _ in range(sink)]
    residual = [[graph[i][j] for j in range(len(graph[i]))] for i in range(sink)]
    max_flow = 0
        ...
        for j in range(ver):
            if not visited[j][i]:
                if i != j and i != s and j != s:
                    visited[i][j] = True
                    visited[j][i] = True
                    graph_c[i].append([e, 10**20])
                    graph_c[j].append([e, 10**20])
                    graph_c.append([])
