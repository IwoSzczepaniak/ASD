...
def highway( A ):
    ...
    last = graph[-1][0]
    tmp = kruskal(graph, v, e, 0, last)
    a = tmp[0]
    b = tmp[1]
    wynik = b - a

    while a < b:
        tiem = kruskal(graph, v, e, a, b-1)
        if tiem[2]:
            b -= 1
            n_wynik = tiem[1] - tiem[0]
            wynik = min(wynik, n_wynik)
        else:
            break

    while a < b:
        tiema = kruskal(graph, v, e, a + 1, b)
        if tiema[2]:
            a += 1
            n_wynik = tiema[1] - tiema[0]
            wynik = min(wynik, n_wynik)
        else:
            break
